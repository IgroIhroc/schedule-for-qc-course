...
describe('renderScheduleTable helpers', () => {
    ...
    test('checkSemesterEnd returns true for past date', () => {
        expect(checkSemesterEnd('01/01/2000')).toBe(true);
    });

    test('checkSemesterEnd returns false for future date', () => {
        const future = '12/12/2999';
        expect(checkSemesterEnd(future)).toBe(false);
    });

    test('isWeekOdd returns true for odd numbers', () => {
        expect(isWeekOdd(1)).toBe(true);
        expect(isWeekOdd(3)).toBe(true);
    });

    test('isWeekOdd returns false for even numbers', () => {
        expect(isWeekOdd(2)).toBe(false);
        expect(isWeekOdd(4)).toBe(false);
    });

    test('getWeekParity returns number >= 0', () => {
        const result = getWeekParity('01/01/2020');
        expect(typeof result).toBe('number');
    });

    test('getWeekParity handles date objects', () => {
